<script lang="ts">
	...
	let { size = 20, color = '#4C74FF' }: { size?: number; color?: string } = $props();
</script>

<svg width={size} height={size} viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
	<!-- DIVERGENCE(upstream): upstream's inline <style> (inside the <svg>) moves to the
	component's top-level scoped <style>; the keyframes/classes are otherwise verbatim. -->
